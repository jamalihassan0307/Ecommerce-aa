        ...
        if (index !== -1) {
            ...
            if (quantityDiff < 0) {
                const totalInCarts = users.reduce((total, user) => {
                    const cartItem = user.cart?.find(item => item.productId === parseInt(productId));
                    return total + (cartItem?.quantity || 0);
                }, 0);

                if (totalInCarts > newProduct.quantity) {
                    showNotification('Cannot reduce quantity below amount in customer carts!');
                    return;
                }
            }
...
function updateCartQuantity(cartItemId, change) {
    ...
    const cartItemIndex = users[userIndex].cart.findIndex(item => item.id === cartItemId);
